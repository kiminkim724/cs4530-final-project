...
function SpotifySearchScreen() {
    ...
    return (
        <div>
            <h1 className="text-center">Search Spotify: "{searchTerm}"</h1>
                ...
                <Row className="mx-2 row row-cols-4">
                    {(() => {
                        switch (searchType) {
                                ...
                                return results.map(result => (
                                    <Link to={`/spotify/album/${result.id}`}>
                                        <Card key={result.id} className="bg-secondary">
                                            ...
                                            <Card.Body>
                                                <Card.Title className="text-dark">{result.name}</Card.Title>
                                                <Link to={`/spotify/artist/${result.artists[0].id}`}>
                                                    <Card.Subtitle className="text-white">{result.artists[0].name}</Card.Subtitle>
                                                </Link>
                                            </Card.Body>
                                        </Card>
                                    </Link>
                            ...
                            case 'artist':
                                return results.map(result => (
                                    <Link to={`/spotify/artist/${result.id}`}>
                                        <Card key={result.id} className="bg-secondary">
                                            <Card.Img src={result.images[0].url} />
                                            <Card.Body>
                            ...
                            case 'track':
                                return results.map(result => (
                                    <Link to={`/spotify/album/${result.album.id}`}>
                                        <Card key={result.id} className="bg-secondary">
                                            <Card.Img src={result.album.images[0].url} />
                                            <Card.Body>
                                                <Card.Title className="text-dark">{result.name}</Card.Title>
                                                <Link to={`/spotify/artist/${result.artists[0].id}`}>
                                                    <Card.Subtitle className="text-white">{result.artists[0].name}</Card.Subtitle>
